...
: _initialScoutSet(false), _initialDefenceSet(false)
{
...
void GameCommander::update()
{
	...
	_timerManager.startTimer(TimerManager::Defence);
	_Defence_Manager.update(_DefenceUnits);
	_timerManager.stopTimer(TimerManager::Defence);
...
void GameCommander::drawGameInformation(int x, int y)
{
	...
	y += 12;
	BWAPI::Broodwar->drawTextScreen(x, y, "\x04Turtle");
}
...
{
	return _combatUnits.contains(unit) || _scoutUnits.contains(unit) || _DefenceUnits.contains(unit);
}

// validates units as usable for distribution to various managers 单位有效的确认,单位实际存在
...
void GameCommander::setScoutUnits()
{
	using namespace std;
	char red = '\x08';
	int State_Carry_Minerals, State_Carry_Gas;
	BWAPI::Unit workerScout=0;
...
//11-7-------------sets defence units to be passed to DefenceCommander
void GameCommander::setDefenceUnits()
{
	using namespace std;
	char red = '\x08';
	int State_Combat;
	int Defence_Size;
	int id;
	BWAPI::Unit DragonDefence = 0;

	Defence_Size = _DefenceUnits.size();

	for (auto & unit : _DefenceUnits)
	{
		if (!UnitUtil::IsValidUnit(unit))
		{
			_initialDefenceSet = false;
		}
	}

	if (_DefenceUnits.size() == 5 && !_initialDefenceSet)
		_DefenceUnits.clear();

	// if we haven't set a complete defence alley, do it
	if (_DefenceUnits.size()<5 || !_initialDefenceSet)
	{
		for (auto & unit : _validUnits)
		{
			if (!isAssigned(unit) && UnitUtil::IsDefenceUnit(unit) && !unit->getType().isWorker())
			{
				id = unit->getID(); //--test
				assignUnit(unit, _DefenceUnits);
				if (_DefenceUnits.size() == 5)
				{
					_initialDefenceSet = true;
					_Defence_Manager.setWorkerDefence(_DefenceUnits);
					break;
				}
...
// 11.3------------sets combat units to be passed to CombatCommander
...
BWAPI::Unit GameCommander::getClosestWorkerToTarget(BWAPI::Position target)
{
	...
	for (auto & unit : _validUnits)
	{
		...
		{
			double dist = unit->getDistance(target);
			if ((!closestUnit || dist < closestDist) && !unit->isCarryingMinerals() && !unit->isCarryingGas() )
			{
				closestUnit = unit;
				closestDist = dist;
			}
...
void GameCommander::assignUnit(BWAPI::Unit unit, BWAPI::Unitset & set)
{
	...
	else if (_DefenceUnits.contains(unit)){ _DefenceUnits.erase(unit); }
