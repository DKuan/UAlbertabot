...
//--------11.7-------¿ªÊ¼defence---------
void WorkerManager::handleDefenceWorkers()
{
	BWAPI::Position Aim_Position;

	for (auto & worker : workerData.getWorkers())
	{
		UAB_ASSERT(worker != nullptr, "Worker was null");

		if (workerData.getWorkerJob(worker) == WorkerData::Defence)
		{
			BWAPI::Broodwar->drawCircleMap(worker->getPosition().x, worker->getPosition().y, 4, BWAPI::Colors::Yellow, true);
			Aim_Position = BWAPI::Broodwar->getMousePosition();
			Micro::SmartMove(worker, Aim_Position);
...
//--------11.7-------set a worker to defence---------
void WorkerManager::setDefenceWorker(BWAPI::Unit worker)
{
	UAB_ASSERT(worker != nullptr, "Worker was null");

	workerData.setWorkerJob(worker, WorkerData::Defence, nullptr);
}
